...
- 从 ACP 后端读取基础 slash command、skill 和 MCP surface chips。
...
3. 使用 `Reasonix: New Session` 或聊天视图里的 `+` 按钮启动新会话。
4. 输入 prompt，通过发送按钮或 `Cmd/Ctrl+Enter` 发送。
5. 使用 command palette 或 editor context menu 中的 `Reasonix: Send Selection` 发送当前编辑器上下文。
...
`npm run test:vscode` 使用 `@vscode/test-electron` 和 fake ACP server，在不发起真实模型调用的前提下验证 extension activation、chat command flow、session startup 和 editor-context sending。
